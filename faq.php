...
<p>The colored indicators are there to act as a guide on how fresh the pricing data is. As <strong>lpStore</strong> relies solely on the EMDR network for pricing; this data may or may not be up to date. At this time, <strong>lpStore</strong> is not collecting historic market data for items, opting to just collect information on what's currently on the market. As <strong>lpStore</strong> does not collect historic data, it would be easy for someone to manipulate the market to the point where the actual value of the item is skewed, even if the market data is less than 24 hours old. If possible, double check the market history in Jita</p>
<dl id='faq-dl'>
